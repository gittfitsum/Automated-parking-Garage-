//************AUTOMATED PARKING GARAGE SYSTEM USING INFRARED SENSOR************//
//***A PROJECT CODE TO THE FEDERAL TECHNICAL AND VOCATIONAL EDUCATION AND TRAINING IINSTITUTE***//
//*IN PARTIAL FULFILMENT FOR THE DEGREE OF BACHELER OF SCINCE IN ELECTRONICS COMMUNICATION TECHNOLOGY*//
...
void setup()
{
  ...
  pinMode(slot1_l, OUTPUT);//Setting slot_1 led as an output
  pinMode(slot2_l, OUTPUT);//Setting slot_2 led as an output
  pinMode(gate_grn, OUTPUT);//Setiting gate_grn led as an output
  pinMode(gate_red, OUTPUT);//Setiting gate_red led as an output
...
void loop()
{
  ...
  { Serial.println("Welcome");
    ...
    digitalWrite(slot2_l, LOW);
    digitalWrite(gate_red, LOW);
    digitalWrite(gate_grn, HIGH); //gate green or yellow light will blink
    delay(100);
